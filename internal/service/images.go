...
func (s *Service) ProcessImage(config model.Message) error {
	format, err := checkFormat(config.ContentType)
	if err != nil {
		return err
	}
...
func (s *Service) addWatermark(fileName, format, watermarkText string) error {
	input, err := os.Open(s.originDirName + "/" + fileName)
	if err != nil {
	...
	output, err := os.OpenFile(s.processedDirName+"/"+fileName, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
